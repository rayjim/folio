...
| **Electron desktop app** | Runs as a native installable app on Windows and macOS via Electron — no browser needed; native `fs` replaces the File System Access API; all dialogs use a custom in-app modal (works with Electron's `contextIsolation`); folder path persisted in `userData` for reliable auto-load on every launch |
| **Multi-window conflict detection** | When multiple Electron windows share the same data folder, conflicts are caught on save and on the 3-second poller — a dialog lets you choose "Load Latest" or "Keep Mine" |
